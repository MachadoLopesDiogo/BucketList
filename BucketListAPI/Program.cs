...
builder.Services.AddDbContext<BucketListDbContext>(options => options.UseMySql(connectionString, ServerVersion.AutoDetect(connectionString)));
...
app.MapGet("bucketlist", async (BucketListDbContext db) =>
{
...
{
    var exist = await db.Bucketlistitems.AnyAsync(pbl => pbl.NameBucketListItem == itemName);
    if (exist)
    {
        return Results.Conflict("Item already exist");
    }
